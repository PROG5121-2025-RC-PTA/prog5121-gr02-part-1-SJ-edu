...
public class UserInputTest{
    ...
    public UserInputTest(String username, String password, String phoneNumber) {
    ...
    public String registerUser() {
        if (!isValidUsername(username)) {
            return "Username is incorrectly formatted, please ensure it contains an underscore and is no more than five characters in length.";
        }
        if (!isValidPassword(password)) {
            return "Password does not meet complexity requirements. It must be at least eight characters long, contain a capital letter, a number, and a special character.";
        }
        if (!isValidPhoneNumber(phoneNumber)) {
            return "Cell phone number is incorrectly formatted. It must include the international country code and be 10 digits long.";
        }
